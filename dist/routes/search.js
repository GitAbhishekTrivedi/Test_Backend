...
exports.searchRouter.get("/", async (req, res, next) => {
    try {
        ...
        const result = await db_1.default.all_users.findMany({
            where: {
                OR: [
                    {
                        name: {
...
const calculateSpamLikelihood = (spamCount, nonSpamCount) => {
...
const isAContact = async (userA, userB) => {
    ...
    const data = await db_1.default.contact.findFirst({
        where: {
...
exports.searchRouter.get("/:id", async (req, res, next) => {
    ...
    const user = await db_1.default.all_users.findFirst({
        where: {
            original_id: id,
            type: type,
        },
